...
function renderToTwoType(chartId, backcolor, title, cateList, xStep,
		yLeftTitle, yRightTitle, dataList, colors, typeList, chartYAxisList,
		chartLegendList, showlegend, height, width, gridLineColor) {
	var seriesList = [];
...
// 双轴大图，参数与上同
function renderToBigTwoType(chartId, backcolor, title, cateList, xStep,
		yLeftTitle, yRightTitle, dataList, colors, typeList, chartYAxisList,
		chartLegendList) {
	var seriesList = [];
	var legendList = chartLegendList;
...
function renderToCommonChart(chartid, centertitle, step, cataList,
		gridLineDashStyle, ytitle, ySymbol, showLegend, parallelcolor,
		seriesList, backcolor, height, width, gridLineColor) {
	$('#' + chartid).highcharts({
		chart : {
			backgroundColor : backcolor,// 背景色：整个表
			height : height,// 调节上下长度
...
// 大图
function renderToBigCommonChart(chartid, centertitle, step, cataList,
		gridLineDashStyle, ytitle, ySymbol, showLegend,
		// 比较线色 数据 背景色
...
function renderToRing(chartid, charttype, categories, colorsArr, datas,
		legendLayout, legendAlign, backcolor,
		// 标题
	...
	$('#' + chartid).highcharts({
				...
				title : {
					text : '<span style="font-size:14px;">' + centerTitle
							+ '</span>',
					style : {
						color : '#C0D0E0'
					}
...
// 大圆环
function renderToBigRing(chartid, charttype, categories, colorsArr, datas,
		legendLayout, legendAlign, backcolor,
		// 标题
		centerTitle) {
	var cates = categories, cArr = colorsArr, data = datas;
...
// 数据处理
function getSeriesList(charttype, connectLineWidth, lineWidth, legendList,
		colorList, fillColorList, radiusList, symbol, referencevalue,
		referencetype, referencelegendname, referencecolor, dataList) {
	var seriesList = [];
	...
	if (referencevalue != '') {
		...
		try {
			...
			seriesList.push({
						...
						marker : {
							radius : 0, // 曲线点半径，默认是4
							symbol : 'circle' // 曲线点类型："circle", "square",
							// "diamond",
							// "triangle","triangle-down"，默认是"circle"
						},
						data : referenceList
...
function renderToSimpleChart(id, legend, cataList, xStep, fillColor, yTitle,
		title, chartcolor, charttype, data, showLegend) {
	$('#' + id).highcharts({
				chart : {
					type : charttype,
					width : 556,
...
// 绘制多线图--设备监测
function getSBJCChart(id, legend, cataList, xStep, fillColor, yTitle, title,
		chartColor, charttype, dataList, showLegend) {
	var data = [];
	data.push({
				name : legend,
				data : dataList
	...
	$("#" + chartid).highcharts({
		...
		series : [{
			...
			data : [80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80,
					80, 80, 80, 80, 80, null, null, null, null],
			type : 'column',
			tooltip : {
			...
			data : [null, null, null, null, null, null, null, 75, 75, 75, 75,
					75, 75, 75, 75, 75, 75, 75, 75, 75, null, null, null, null],
			type : 'column',
			tooltip : {
				valueSuffix : '%'
			}
			...
			name : '冷机3#',
			data : [null, null, null, null, null, null, null, 55, 90, 55, 55,
					55, 55, 55, 55, 55, 55, 55, 55, 55, 80, 80, 80, 80],
			type : 'column',
			tooltip : {
				valueSuffix : '%'
			}
			...
			name : '冷机4#',
			data : [null, null, null, null, null, null, null, 60, 60, 60, 60,
					60, 60, 60, 60, 60, 60, 60, 60, 60, 55, 55, 55, 55],
			type : 'column',
			tooltip : {
				valueSuffix : '%'
			}
			...
			name : 'COP',
			data : [2.5, 3.0, 3.1, 3.2, 3.0, 2.8, 2.5, 2.6, 2.4, 2.9, 3.1, 3.2,
					3.3, 3.5, 3.4, 3.3, 3.2, 3.1, 3.1, 3.2, 3.0, 2.8, 2.7, 2.6],
			type : 'spline',// 平滑线
			yAxis : 1
				// 以编号为‘1’的y轴为基准
		}]
...
function getYXSJChart(chartId) {
	$('#' + chartId).highcharts({
		...
		tooltip : {
			formatter : function() {
				return this.x + '点状态:<b>' + this.series.name + '</b>';
			}
		},
		...
		series : [{
			name : '开启',
			data : [10, null, 10, null, 10, 10, null, 10, null, 10, 10, null,
					10, null, 10, 10, null, 10, null, 10, 10, null, 10, null]
		}, {
			name : '关闭',
			data : [null, 10, null, 10, null, null, 10, null, 10, null, null,
					10, null, 10, null, null, 10, null, 10, null, null, 10,
					null, 10]
		}]
	});
}
...
function getSBXQ_YXSJchart(id) {
	$("#" + id).highcharts({
		...
		series : [{
					...
					name : '关闭',
					data : [null, null, null, 10, 10, null, null, 10, 10, null,
							null],
					connectEnds : true,
					zIndex : 2
				}],
