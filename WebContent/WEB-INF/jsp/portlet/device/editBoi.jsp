...
<!-- 
 ...
 -->
 <div id="msg"></div>
<form:form commandName="boi" method="post" cssClass="editform"
	action="#">
	...
	<div class="div-table">
		<div class="div-table-row">
			<div class="div-table-2col">
				<div class="fitem">
					<form:label path="hc">额定供热量：</form:label>
					<form:input path="hc" onkeyup="value=value.replace(/[^\d.]/g,'') "onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d.]/g,''))"  />(必须为数值)
				</div>
			</div>
			<div class="div-table-2col">
				<div class="fitem">
					<form:label path="frFue">燃料耗量：</form:label>
					<form:input path="frFue" onkeyup="value=value.replace(/[^\d.]/g,'') "onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d.]/g,''))"  />(必须为数值)
				</div>
			</div>
		</div>
		<div class="div-table-row">
			<div class="div-table-2col">
				<div class="fitem">
					<form:label path="THwS">热水供水温度：</form:label>
			...
			<div class="div-table-2col">
				<div class="fitem">
					<form:label path="THwR">热水回水温度：</form:label>
		...
		<div class="div-table-row">
			<div class="div-table-2col">
				<div class="fitem">
					<form:label path="frHw">热水流量：</form:label>
...
<script language="javascript">
	...
	*/
	function <portlet:namespace/>saveForm() {
		var resurl = '<portlet:resourceURL id="submitBoiForm"></portlet:resourceURL>';
		$("#boi").form('submit', {
			url: resurl,
			onSubmit : function() {
			...
			success: function(result){
				...
				if (result.success){
					...
					if(isnew=='true') {
						...
						redirect += "&<portlet:namespace/>action=editBoiForm&<portlet:namespace/>id="+result.msg; 
	...
	function <portlet:namespace/>remove() {
		if($("#isnew").val()=="false") {
			$.messager.confirm('Confirm','确定要删除吗?',function(r){
				 if (r){
					var removeUrl = "<portlet:resourceURL id='removeBoi'></portlet:resourceURL>";
					$.post(removeUrl,{id:'${boi.id}'},function(result){
						if (result.success){
							alert("数据删除成功!");
