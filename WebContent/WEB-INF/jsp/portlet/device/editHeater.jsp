...
<!-- 
 ...
 -->
 <div id="msg"></div>
<form:form commandName="heater" method="post" cssClass="editform"
	action="#">
...
<script language="javascript">
	...
	*/
	function <portlet:namespace/>saveForm() {
		var resurl = '<portlet:resourceURL id="submitHeaterForm"></portlet:resourceURL>';
		$("#heater").form('submit', {
			url: resurl,
			onSubmit : function() {
			...
			success: function(result){
				...
				if (result.success){
					...
					if(isnew=='true') {
						...
						redirect += "&<portlet:namespace/>action=editHeaterForm&<portlet:namespace/>id="+result.msg; 
	...
	function <portlet:namespace/>remove() {
		if($("#isnew").val()=="false") {
			$.messager.confirm('Confirm','确定要删除吗?',function(r){
				 if (r){
					var removeUrl = "<portlet:resourceURL id='removeHeater'></portlet:resourceURL>";
					$.post(removeUrl,{id:'${heater.id}'},function(result){
						if (result.success){
							alert("数据删除成功!");
