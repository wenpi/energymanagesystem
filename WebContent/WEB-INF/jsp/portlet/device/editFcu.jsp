...
<!-- 
 ...
 -->
 <div id="msg"></div>
<form:form commandName="fcu" method="post" cssClass="editform"
	action="#">
...
<script language="javascript">
	...
	*/
	function <portlet:namespace/>saveForm() {
		var resurl = '<portlet:resourceURL id="submitFcuForm"></portlet:resourceURL>';
		$("#fcu").form('submit', {
			url: resurl,
			onSubmit : function() {
			...
			success: function(result){
				...
				if (result.success){
					...
					if(isnew=='true') {
						...
						redirect += "&<portlet:namespace/>action=editFcuForm&<portlet:namespace/>id="+result.msg; 
	...
	function <portlet:namespace/>remove() {
		if($("#isnew").val()=="false") {
			$.messager.confirm('Confirm','确定要删除吗?',function(r){
				 if (r){
					var removeUrl = "<portlet:resourceURL id='removeFcu'></portlet:resourceURL>";
					$.post(removeUrl,{id:'${fcu.id}'},function(result){
						if (result.success){
							alert("数据删除成功!");
