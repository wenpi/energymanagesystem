...
<!-- 
 ...
 -->
 <div id="msg"></div>
<form:form commandName="vav" method="post" cssClass="editform"
	action="#">
	...
	<div class="div-table">
		<div class="div-table-row">
			...
			<div class="div-table-2col">
				<div class="fitem">
					<form:label path="cc">额定冷量：</form:label>
					<form:input path="cc" onkeyup="value=value.replace(/[^\d.]/g,'') "onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d.]/g,''))"  />(必须为数值)
				</div>
			</div>
		</div>
		<div class="div-table-row">
			<div class="div-table-2col">
				<div class="fitem">
					<form:label path="hc">额定热量：</form:label>
			...
			<div class="div-table-2col">
				<div class="fitem">
					<form:label path="fr">额定风量：</form:label>
		...
		<div class="div-table-row">
			<div class="div-table-2col">
				<div class="fitem">
					<form:label path="powerHt">电加热功率：</form:label>
...
<script language="javascript">
	...
	*/
	function <portlet:namespace/>saveForm() {
		var resurl = '<portlet:resourceURL id="submitVavForm"></portlet:resourceURL>';
		$("#vav").form('submit', {
			url: resurl,
			onSubmit : function() {
			...
			success: function(result){
				...
				if (result.success){
					...
					if(isnew=='true') {
						...
						redirect += "&<portlet:namespace/>action=editVavForm&<portlet:namespace/>id="+result.msg; 
	...
	function <portlet:namespace/>remove() {
		if($("#isnew").val()=="false") {
			$.messager.confirm('Confirm','确定要删除吗?',function(r){
				 if (r){
					var removeUrl = "<portlet:resourceURL id='removeVav'></portlet:resourceURL>";
					$.post(removeUrl,{id:'${vav.id}'},function(result){
						if (result.success){
							alert("数据删除成功!");
