	...
	<div class="chart">
		<div class="look_other">
			...
			<div class="tool_groupt">
				...
				<div class="has_border_radius_right downLoad_btn" onclick="<portlet:namespace />expToExcel3()"></div>
			</div>
		</div>
		<div class="sub_content chart_content onlychart <portlet:namespace/>showContent3" id="<portlet:namespace />consumption3"></div>
...
/* 	var <portlet:namespace />centerTitle3 = ""; //图表居中标题
	...
	function <portlet:namespace />getLeftChart3(tfrom,name,id,ispd,legendList) {
			...
			.post(
					'<portlet:resourceURL id="getCommonChartData"></portlet:resourceURL>',
					{
						...
						if (<portlet:namespace />choose_type3 == "day"){
							...
							/* // 保存图表标题
							<portlet:namespace />centerTitle3 = <portlet:namespace />c_title3 + "/" + cataList[0] + "~~" + <portlet:namespace />c_title3 + "/" + cataList[cataList.length-1]//保存居中标题 */
							// 往后台返回的X轴列表中添加周一~周日
						...
						//该方法写在\tomcat-3.0.29\webapps\ROOT\html\company\scripts目录下的autoRenderToCharts.js中，用来生成图表
						autoRenderToMoreCharts("<portlet:namespace />consumption3",// 对应的渲染位置 
			...
			$.post(
					'<portlet:resourceURL id="getReportDataList"></portlet:resourceURL>',
					{
						...
						for(var i = 0 ;i<showCataList[0].length;i++){
							content +="<tr>" + (<portlet:namespace />choose_type3 == "day"?("<td>" + <portlet:namespace />start_date1 + "</td>"):"") + "<td>" + showCataList[0][i] + "</td>";
		...
		*/
		function <portlet:namespace />confirm3(){
			// 渲染左侧图表
			<portlet:namespace />getLeftChart3(<portlet:namespace />times3.join(),<portlet:namespace />chart_name3,<portlet:namespace />chart_id3,<portlet:namespace />chart_ispd3,<portlet:namespace />chartLegends3);
			// 获取网格数据
			<portlet:namespace />getForm3();
		}
