...
<!-- 
	...
	LastModified Date: 2013-12-9 上午10点11分
	Description: 能耗报表-建筑总能耗
 -->
...
<div class="charts">
	<div class="chart">
		...
		<div
			class="sub_content chart_content <portlet:namespace/>showContent1">
			<%-- <div class="double_chartTitle"><p style="font-family:微软雅黑;font-size: 16px;color: #274b6d;">${tbinfo.chartTitle1}</p> --%>
			<div id="<portlet:namespace />consumptionLeft1" class="double_chart"></div>
			<!-- </div> -->
			<div id="<portlet:namespace />consumptionRight1" class="double_chart"></div>
		</div>
		<div
...
<script type="text/javascript">
	...
	var <portlet:namespace />centerTitle1 = "${tbinfo.chartTitle1}"; //图表居中标题
	...
	var <portlet:namespace />chart_name2 = '${tbinfo.chart_name2}'; // 右图对应的name
	var <portlet:namespace />chart_id2 = '${tbinfo.chart_id2}'; // 右图对应的id
	var <portlet:namespace />chart_ispd2 = '${tbinfo.chart_ispd2}'; // 右图对应的ispd
	...
	function <portlet:namespace />selectdate1(){
		...
		WdatePicker({
			...
			onpicked : function(dp) {
				...
				//获取右图name,id,ispd信息
				<portlet:namespace />chart_name2 = '${tbinfo.chart_name2}'; // 对应的name
				<portlet:namespace />chart_id2 = '${tbinfo.chart_id2}'; // 对应的id
				<portlet:namespace />chart_ispd2 = '${tbinfo.chart_ispd2}'; // 对应的ispd
	...
	function <portlet:namespace />getLeftChart1(tfrom,name,id,ispd,legendList) {
		$
			.post(
					...
					autoRenderToMoreCharts("<portlet:namespace />consumptionLeft1",// 对应的渲染位置 
		...
		$(function() {
			...
			if(showBuild == "show"){
				$("#<portlet:namespace />choose_build1").show(); // 显示选择建筑项
			}else{ 
				$("#<portlet:namespace />choose_build1").hide(); // 隐藏选择建筑项
			}
			//调用图表加载方法，加载相应图表数据
		...
		*/
		function <portlet:namespace />confirm1(){
			...
			// 渲染右侧图表
			<portlet:namespace />getRightChart1(<portlet:namespace />start_date1,<portlet:namespace />chart_name2,<portlet:namespace />chart_id2,<portlet:namespace />chart_ispd2);
			// 获取网格数据
		...
		function <portlet:namespace />loadData1(){
			...
			if($("#<portlet:namespace />table_chart_switch div:eq(0)").hasClass("btn_press")){
				if(<portlet:namespace />loadChart1 == 0){
					...
					// 渲染右侧图表
					<portlet:namespace />getRightChart1(<portlet:namespace />start_date1,<portlet:namespace />chart_name2,
							<portlet:namespace />chart_id2,<portlet:namespace />chart_ispd2);
