	...
	<div class="look_other">
		...
		<div class="date_comparer">
			<input id="<portlet:namespace/>comparing3" type="text"
				...
				onclick="<portlet:namespace/>comparing3()" /> <span class="comp_btn">加入对比</span>
...
<div class="detail">
	<div>
		<table class = "unit_ele">
			<caption>单位面积总用电量</caption>
			<tbody>
				<tr>
...
var <portlet:namespace/>choose_name3 = "${energyUseAnalysis.choose_name3}";//对应的数据name
var <portlet:namespace/>choose_id3 = "${energyUseAnalysis.choose_id3}";//对应的数据Id
var <portlet:namespace/>ispd3 = "${energyUseAnalysis.ispd3}";//对应的ispd
var <portlet:namespace/>legendList3 = "${energyUseAnalysis.chartLegendList_3}";//图例列表
...
if(publicTime != ""){
	...
	if ("${energyUseAnalysis.defaulttime3}" == "") {//默认显示当前时间
		var nowdate = new Date();
		<portlet:namespace/>start_date3 = nowdate.getFullYear() + "-" + (nowdate.getMonth() + 1)
				+ "-" + nowdate.getDate();
	} else {
		<portlet:namespace/>start_date3 = "${energyUseAnalysis.defaulttime3}";//首选项数据，默认时间
	}
}
	...
	WdatePicker({
		...
		onpicked : function(dp) {
			...
			if (type == "day") {
				...
				$(".unit_ele td > span ").text("日"); // 文字
				$(".unit_ele > tbody > tr:eq(3) > td:eq(0)").text("同比昨日"); // 文字
			} else if (type == "week") {
				<portlet:namespace/>step3 = "1";
				<portlet:namespace/>outsideStr3 = "该周";
				$(".unit_ele td > span ").text("周"); // 文字
				$(".unit_ele > tbody > tr:eq(3) > td:eq(0)").text("同比上周"); // 文字
			} else if (type == "month") {
				<portlet:namespace/>outsideStr3 = _time.getDateStr('yyyy') + "年"
						+ _time.getDateStr('MM') + "月";
				<portlet:namespace/>step3 = "5";
				$(".unit_ele td > span ").text("月"); // 文字
				$(".unit_ele > tbody > tr:eq(3) > td:eq(0)").text("同比上月"); // 文字
			} else if (type == "year") {
				<portlet:namespace/>outsideStr3 = _time.getDateStr('yyyy');
				<portlet:namespace/>step3 = "2";
				$(".unit_ele td > span ").text("年"); // 文字
				$(".unit_ele > tbody > tr:eq(3) > td:eq(0)").text("同比去年"); // 文字
			}
			<portlet:namespace/>choose_name3 = '${energyUseAnalysis.choose_name3}'; // 对应的name
			<portlet:namespace/>choose_id3 = '${energyUseAnalysis.choose_id3}'; // 对应的id
			<portlet:namespace/>ispd3 = '${energyUseAnalysis.ispd3}'; // 对应的ispd

			var times = "";
			// 根据name的个数，来为att添加对应个数的值
			var legend = "${energyUseAnalysis.chartLegendList_3}".replace(/'/g,"").split(",");//
...
function <portlet:namespace/>comparing3(){
	...
	WdatePicker({
		...
		onpicked : function(dp) {
			...
			var times = [];
			// 根据name的个数，来为att添加对应个数的值
			var chartLegendList = [${energyUseAnalysis.chartLegendList_3}];
			...
			<portlet:namespace />exp_start_date3 =times;// 导出到excel中的时间 
			<portlet:namespace />choose_name3= '${energyUseAnalysis.choose_name3},${energyUseAnalysis.choose_name3}'; // 对应的name
			<portlet:namespace />choose_id3 = '${energyUseAnalysis.choose_id3},${energyUseAnalysis.choose_id3}'; // 对应的id
			<portlet:namespace />ispd3 = '${energyUseAnalysis.ispd3},${energyUseAnalysis.ispd3}'; // 对应的ispd
			for(var i=0;i<chartLegendList.length;i++){
...
function <portlet:namespace/>getChart3(tfrom, name, id, ispd, legend, showLegend,
	...
	$.post("<portlet:resourceURL id='getCommonChartData'></portlet:resourceURL>", {
		...
		id : id,
		ispd : ispd,
		decimals : '${energyUseAnalysis.decimals3}',//显示小数位数，首选项传
	...
	//选取时间段
		...
		if("${energyUseAnalysis.xStep_3}"!=''){
			<portlet:namespace/>step1="${energyUseAnalysis.xStep_3}";
		}
		//图例处理
		if(isNotCompare == "yes"){
			var legendList="${energyUseAnalysis.chartLegendList_3}".split(",");
			legend=legend.split(",");
			var temp=legend[1];
		...
		var chartColorList = [${energyUseAnalysis.chartColorList_3}];
		var chartYAxisList = [${energyUseAnalysis.chartYAxisList}];
		var chartTypeList=['${energyUseAnalysis.chartType_3_1}','${energyUseAnalysis.chartType_3_2}','${energyUseAnalysis.chartType_3_1}','${energyUseAnalysis.chartType_3_2}'];
		renderToBigTwoType("<portlet:namespace />chartArea3", 
				"${energyUseAnalysis.backgroundColor_3}",
				'',
				cateList,
				<portlet:namespace/>step3,
				"${energyUseAnalysis.yLeftTitle_3}",
				"${energyUseAnalysis.yRightTitle_3}",
				dataList,
				chartColorList,
				chartTypeList,
				chartYAxisList,
...
function <portlet:namespace/>expToExcel3(){
	...
	var expColumnName = "${energyUseAnalysis.expColumnName3}".replace(/'/g,"");
	// 导出excel的Url
	var expUrl = "<portlet:resourceURL id='expTotalEnergyDataToExcel'></portlet:resourceURL>"
			+ "&excelName="
			+ "${energyUseAnalysis.title3}"
			+"&hours="
			...
			+ isNotCompare
			+ "&expColumnName="
			+ (expColumnName==""?"${energyUseAnalysis.expColumnName3}".replace(/'/g,""):expColumnName);
	var removeUrl = '<portlet:resourceURL id="deleteExcelDataTempFile"><portlet:param name="fileName" value="${energyUseAnalysis.title3}" /></portlet:resourceURL>';
	var hSrc = "http://" + document.location.host
			+ "/energymanagesystem/uploadfiles/" + "${energyUseAnalysis.title3}"
			+ ".xls";
	$.ajax({
		type : "POST",
		url : expUrl,
...
function <portlet:namespace />getRightData3(){
	$.ajax({
		...
		success : function(result) {
			...
			$(".unit_ele > tbody > tr:eq(0) > td:eq(1)").text(parseFloat(data[0][0])); // max
			$(".unit_ele > tbody > tr:eq(0) > td:eq(3)").text(maxtime); // maxTime
			$(".unit_ele > tbody > tr:eq(1) > td:eq(1)").text(parseFloat(data[1][0]));// min
			$(".unit_ele > tbody > tr:eq(1) > td:eq(3)").text(mintime); // minTime
			$(".unit_ele > tbody > tr:eq(2) > td:eq(1)").text(parseFloat(data[2][0])); // sum
			$(".unit_ele > tbody > tr:eq(3) > td:eq(1)").text(returnUpOrDown(data[3][0]) + returnPositiveVal(data[3][0],"0"));// change
		},
		error : function(result) {
