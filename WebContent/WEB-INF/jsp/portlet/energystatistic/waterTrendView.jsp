...
<div class="div-table">
	...
	<div class="parameter_1 water"
		...
		<table class="client_para">
			<tr>
				<td><span id="<portlet:namespace />datavalue4_1">20</span><sup>kg</sup></td>
				<td><span id="<portlet:namespace />datavalue4_2">7</span><sup>%</sup><span id="<portlet:namespace />upOrDown4"></span></td>
			</tr>
			<tr>
				<td><span id="<portlet:namespace />datavalue4_3">当日用水量</span></td>
	...
	<div class="timediv">
		...
		<div id="<portlet:namespace />yearddd4" onclick="<portlet:namespace />outputToExcel4()" class="tdatt output">输出</div>
		...
		<!-- class="basicchartdiv jzzysqs" -->
		<div id="<portlet:namespace />watertrend" class="basicchartdiv"></div>
	</div>
...
function <portlet:namespace />comparetime4() {
	...
	WdatePicker({
		...
		onpicked : function(dp) {
			...
			<portlet:namespace />choose_name4 = '${buildtotalconsumption.choose_name4},${buildtotalconsumption.choose_name4}'; // 对应的name
			<portlet:namespace />choose_id4 = '${buildtotalconsumption.choose_id4},${buildtotalconsumption.choose_id4}'; // 对应的id
			<portlet:namespace />ispd4 = '${buildtotalconsumption.ispd4},${buildtotalconsumption.ispd4}'; // 对应的ispd
	...
	var <portlet:namespace />str1_4 = '当日用水量'; // 存储选择日历后的返回的文字描述
	var <portlet:namespace />str2_4 = '与昨日相比'; // 存储选择日历后的对应的文字描述
	var <portlet:namespace />step4 = "${buildtotalconsumption.xStep_4}"; //存储X轴的显示间隔
	var <portlet:namespace />legendList4 = "${buildtotalconsumption.chartLegendList_4}";// 图例列表
	...
	var <portlet:namespace />choose_name4 = '${buildtotalconsumption.choose_name4}'; // 对应的name
	var <portlet:namespace />choose_id4 = '${buildtotalconsumption.choose_id4}'; // 对应的id
	var <portlet:namespace />ispd4 = '${buildtotalconsumption.ispd4}'; // 对应的ispd
	...
	function <portlet:namespace />selectdate4(type) {
		...
		WdatePicker({
			...
			onpicked : function(dp) {
				...
				if (type == "day"){
					<portlet:namespace />str1_4 =  _time.getDateStr('MM')+"月"+ _time.getDateStr('dd') + "日用水量";
					<portlet:namespace />str2_4 = "与昨日相比";
					<portlet:namespace />step4 = "${buildtotalconsumption.xStep_4}";
				} else if (type == "week") {
					<portlet:namespace />step4 = "1";
					<portlet:namespace />str1_4 = "该周用水量";
					<portlet:namespace />str2_4 = "与上周相比";
				} else if (type == "month") {
					<portlet:namespace />str1_4 = _time.getDateStr('yyyy') + "年" + _time.getDateStr('MM') + "月用水量";
					<portlet:namespace />str2_4 = "与上月相比";
					<portlet:namespace />step4 = "${buildtotalconsumption.xStep_4}";
				} else if (type == "year"){
					<portlet:namespace />str1_4 = _time.getDateStr('yyyy') + "年用水量";
				...
				// 获取图表
				<portlet:namespace />choose_name4 = '${buildtotalconsumption.choose_name4}';
				<portlet:namespace />choose_id4 = '${buildtotalconsumption.choose_id4}';
				<portlet:namespace />ispd4 = '${buildtotalconsumption.ispd4}';
		...
		$
				.post(
						'<portlet:resourceURL id="getCommonChartData"></portlet:resourceURL>',
						{
							...
							id : id,
							ispd : ispd,
							decimals : '${buildtotalconsumption.decimals4}',
							...
							seriesList = returnSeriesList("${buildtotalconsumption.chartType_4_1}",
														  "${buildtotalconsumption.connectLineWidth_4}", // 连接线线宽，为空则默认为2，为0则不显示连接线
														  "${buildtotalconsumption.lineWidth_4}", // 线宽，此为具体曲线点的点线宽  ，为空则默认为2
														  <portlet:namespace />legendList4,
														  "${buildtotalconsumption.chartColorList_4}", // 颜色列表
														  "${buildtotalconsumption.chartFillColorList_4}", // 填充颜色列表
														  "${buildtotalconsumption.chartRadiusList_4}", // 曲线点半径列表,默认为4
														  "${buildtotalconsumption.symbol4}", // 曲线点类型："circle", "square", "diamond", "triangle","triangle-down"，默认是"circle"
														  "", // 参考值  
														  "${buildtotalconsumption.chartType_4_2}", // 参考值的图表类型
														  "${buildtotalconsumption.referenceLegend_4}", // 参考值对应的图例名称
														  "${buildtotalconsumption.referenceColor_4}", // 参考值对应的颜色
														  dataList);
							
							// 将图表渲染到对应的位置上
							renderCompareChart("<portlet:namespace />watertrend",// 对应的渲染位置
											   <portlet:namespace />centerTitle4,// 居中标题
											   <portlet:namespace />step4,// X轴间隔
											   cataList,// X轴数据列表
											   '', // 列表中的线的样式，默认为虚线
											   '${buildtotalconsumption.yLeftTitle_4}',//  Y轴标题
											   '${buildtotalconsumption.ySymbol4}',// Y轴的单位
											   showLegend == '' ? false : showLegend,// 是否显示图例
											   "${buildtotalconsumption.referenceColor_4}",// 参考颜色
											   seriesList);
						}, 'json');
	}
	...
	function <portlet:namespace />setTextAndValue4(fdate,str1_4,str2_4){
		...
		.post(
				...
				{
					from : fdate,
					name : '${buildtotalconsumption.choose_name4}',
					id : '${buildtotalconsumption.choose_id4}',
					ispd : '${buildtotalconsumption.ispd4}',
					att : 'sum,change',
					type : <portlet:namespace />choose_type4
				}, function(data) {
					// 为相应的位置赋值
					...
					// 设置左侧对应位置的文字描述及数据
					$('#<portlet:namespace />datavalue4_1').text(returnPositiveVal(valueList[0],'${buildtotalconsumption.decimals4}'));
					$('#<portlet:namespace />datavalue4_2').text(returnPositiveVal(valueList[1],'${buildtotalconsumption.decimals4}'));
	...
	if("${buildtotalconsumption.defaulttime4}" == ""){
		var nowdate = new Date();
		<portlet:namespace />start_date4 = nowdate.getFullYear() + "-" + (nowdate.getMonth()+1) + "-"
				+ nowdate.getDate();
	} else {
		<portlet:namespace />start_date4 = "${buildtotalconsumption.defaulttime4}";
	}
	...
	function <portlet:namespace />outputToExcel4(){
		...
		var expColumnName4 = "${buildtotalconsumption.expColumnName4}";
		// 导出excel的Url
		var expUrl = "<portlet:resourceURL id='expCommonChartDataToExcel'></portlet:resourceURL>"
				+ "&excelName="
				+ "${buildtotalconsumption.title4}"
				+ "&from="
				+ sdate
				+ "&name="
				...
				+ isNotCompare
				+ "&decimals="
				+ '${buildtotalconsumption.decimals4}'
				+ "&expColumnName="
				+ (expColumnName4==""?"${buildtotalconsumption.title4}":expColumnName4);
		var removeUrl = '<portlet:resourceURL id="deleteExcelDataTempFile"><portlet:param name="fileName" value="${buildtotalconsumption.title4}" /></portlet:resourceURL>';
		var hSrc = "http://" + document.location.host
				+ "/energymanagesystem/uploadfiles/" + "${buildtotalconsumption.title4}"
				+ ".xls";
		$.ajax({
			type : "POST",
			url : expUrl,
