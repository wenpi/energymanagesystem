...
<!-- 
	...
	LastModified Date:
	Description: 能耗统计-建筑总用电趋势
 -->

<div class="div-table">

	<div class="parameter_1 ele"
		style="background-color:${buildtotalconsumption.backgroundColor_3};">
		<div class="page_header">
			<h1>${buildtotalconsumption.title3}</h1>
		</div>
		<h5>${buildtotalconsumption.description_3}</h5>
		<div class="division"></div>
		<table class="client_para">
			<tr>
				<td><span id="<portlet:namespace />datavalue3_1">800</span><sup>kWh</sup></td>
				<td><span id="<portlet:namespace />datavalue3_2">5</span><sup>%</sup><span
					id="<portlet:namespace />upOrDown3"></span></td>
			</tr>
			<tr>
				<td><span id="<portlet:namespace />datavalue3_3">当日用电量</span></td>
	...
	<div class="timediv">
		...
		<div id="<portlet:namespace />electricitytrend" class="basicchartdiv"></div>

		<div class="bottom_link">
			<i><a href="${buildtotalconsumption.linkUrl_1_3}">${buildtotalconsumption.linkUrl_1_3}</a></i><i><a
				href="${buildtotalconsumption.linkUrl_2_3}">${buildtotalconsumption.linkName_2_3}</a></i>
		</div>
...
function <portlet:namespace />comparetime3() {
	...
	WdatePicker({
		...
		onpicked : function(dp) {
			...
			<portlet:namespace />choose_name3 = '${buildtotalconsumption.choose_name3},${buildtotalconsumption.choose_name3}'; // 对应的name
			<portlet:namespace />choose_id3 = '${buildtotalconsumption.choose_id3},${buildtotalconsumption.choose_id3}'; // 对应的id
			<portlet:namespace />ispd3 = '${buildtotalconsumption.ispd3},${buildtotalconsumption.ispd3}'; // 对应的ispd
	...
	var <portlet:namespace />str1_3 = '当日用电量'; // 存储选择日历后的返回的文字描述
	var <portlet:namespace />str2_3 = '与昨日相比'; // 存储选择日历后的对应的文字描述
	var <portlet:namespace />step3 = "${buildtotalconsumption.xStep_3}"; //存储X轴的显示间隔
	var <portlet:namespace />legendList3 = "${buildtotalconsumption.chartLegendList_3}";// 图例列表
	...
	var <portlet:namespace />choose_name3 = '${buildtotalconsumption.choose_name3}'; // 对应的name
	var <portlet:namespace />choose_id3 = '${buildtotalconsumption.choose_id3}'; // 对应的id
	var <portlet:namespace />ispd3 = '${buildtotalconsumption.ispd3}'; // 对应的ispd
	...
	function <portlet:namespace />selectdate3(type) {
		...
		WdatePicker({
			...
			onpicked : function(dp) {
				...
				if (type == "day"){
					<portlet:namespace />str1_3 =  _time.getDateStr('MM')+"月"+ _time.getDateStr('dd') + "日用电量";
					<portlet:namespace />str2_3 = "与昨日相比";
					<portlet:namespace />step3 = "${buildtotalconsumption.xStep_3}";
				} else if (type == "week") {
					<portlet:namespace />step3 = "1";
					<portlet:namespace />str1_3 = "该周用电量";
					<portlet:namespace />str2_3 = "与上周相比";
				} else if (type == "month") {
					<portlet:namespace />str1_3 = _time.getDateStr('yyyy') + "年" + _time.getDateStr('MM') + "月用电量";
					<portlet:namespace />str2_3 = "与上月相比";
					<portlet:namespace />step3 = "${buildtotalconsumption.xStep_3}";
				} else if (type == "year"){
					<portlet:namespace />str1_3 = _time.getDateStr('yyyy') + "年用电量";
				...
				// 获取图表
				<portlet:namespace />choose_name3 = '${buildtotalconsumption.choose_name3}';
				<portlet:namespace />choose_id3 = '${buildtotalconsumption.choose_id3}';
				<portlet:namespace />ispd3 = '${buildtotalconsumption.ispd3}';
		...
		$
				.post(
						'<portlet:resourceURL id="getCommonChartData"></portlet:resourceURL>',
						{
							...
							id : id,
							ispd : ispd,
							decimals : '${buildtotalconsumption.decimals3}',
							...
							seriesList = returnSeriesList("${buildtotalconsumption.chartType_3_1}",
														  "${buildtotalconsumption.connectLineWidth_3}", // 连接线线宽，为空则默认为2，为0则不显示连接线
														  "${buildtotalconsumption.lineWidth_3}", // 线宽，此为具体曲线点的点线宽  ，为空则默认为2
														  <portlet:namespace />legendList3,
														  "${buildtotalconsumption.chartColorList_3}", // 颜色列表
														  "${buildtotalconsumption.chartFillColorList_3}", // 填充颜色列表
														  "${buildtotalconsumption.chartRadiusList_3}", // 曲线点半径列表,默认为4
														  "${buildtotalconsumption.symbol3}", // 曲线点类型："circle", "square", "diamond", "triangle","triangle-down"，默认是"circle"
														  "", // 参考值  
														  "${buildtotalconsumption.chartType_3_2}", // 参考值的图表类型
														  "${buildtotalconsumption.referenceLegend_3}", // 参考值对应的图例名称
														  "${buildtotalconsumption.referenceColor_3}", // 参考值对应的颜色
														  dataList);
							
							// 将图表渲染到对应的位置上
							renderCompareChart("<portlet:namespace />electricitytrend",// 对应的渲染位置
											   <portlet:namespace />centerTitle3,// 居中标题
											   <portlet:namespace />step3,// X轴间隔
											   cataList,// X轴数据列表
											   '', // 列表中的线的样式，默认为虚线
											   '${buildtotalconsumption.yLeftTitle_3}',//  Y轴标题
											   '${buildtotalconsumption.ySymbol3}',// Y轴的单位
											   showLegend == '' ? false : showLegend,// 是否显示图例
											   "${buildtotalconsumption.referenceColor_3}",// 参考颜色
											   seriesList);
						}, 'json');
	}
	...
	function <portlet:namespace />setTextAndValue3(fdate,str1_3,str2_3){
		...
		.post(
				...
				{
					from : fdate,
					name : '${buildtotalconsumption.choose_name3}',
					id : '${buildtotalconsumption.choose_id3}',
					ispd : '${buildtotalconsumption.ispd3}',
					att : 'sum,change',
					type : <portlet:namespace />choose_type3
				}, function(data) {
					// 为相应的位置赋值
					...
					// 设置左侧对应位置的文字描述及数据
					$('#<portlet:namespace />datavalue3_1').text(returnPositiveVal(valueList[0],'${buildtotalconsumption.decimals3}'));
					$('#<portlet:namespace />datavalue3_2').text(returnPositiveVal(valueList[1],'${buildtotalconsumption.decimals3}'));
	...
	if("${buildtotalconsumption.defaulttime3}" != "") {
		<portlet:namespace />start_date3 = "${buildtotalconsumption.defaulttime3}";
	} else if(publicTime != "") {
	...
	function <portlet:namespace />outputToExcel3(){
		...
		var expColumnName3 = "${buildtotalconsumption.expColumnName3}";
		// 导出excel的Url
		var expUrl = "<portlet:resourceURL id='expCommonChartDataToExcel'></portlet:resourceURL>"
				+ "&excelName="
				+ "${buildtotalconsumption.title3}"
				+ "&from="
				+ sdate
				+ "&name="
				...
				+ isNotCompare
				+ "&decimals="
				+ '${buildtotalconsumption.decimals3}'
				+ "&expColumnName="
				+ (expColumnName3==""?"${buildtotalconsumption.title3}":expColumnName3);
		var removeUrl = '<portlet:resourceURL id="deleteExcelDataTempFile"><portlet:param name="fileName" value="${buildtotalconsumption.title3}" /></portlet:resourceURL>';
		var hSrc = "http://" + document.location.host
				+ "/energymanagesystem/uploadfiles/" + "${buildtotalconsumption.title3}"
				+ ".xls";
		$.ajax({
			type : "POST",
			url : expUrl,
