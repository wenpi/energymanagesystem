	...
	<div class="chart">
		<div class="look_other">
			...
			<div class="tool_groupt">
				...
				<div class="has_border_radius_right downLoad_btn" onclick="<portlet:namespace />expToExcel5()"></div>
			</div>
		</div>
		<div class="sub_content chart_content onlychart <portlet:namespace/>showContent5" id="<portlet:namespace />consumption5"></div>
...
/* 	var <portlet:namespace />centerTitle5 = ""; //图表居中标题
	...
	var <portlet:namespace />series_count5 = '${tbinfo.chart_name5}';
	var <portlet:namespace />series_total5 = <portlet:namespace />series_count5.split(",");//保存每组图表的数据列
	var <portlet:namespace />legend_count5 = <portlet:namespace />chartLegends5.split(",").length;//保存图例说明已配置的个数 
	...
	function <portlet:namespace />getLeftChart5(tfrom,name,id,ispd,legendList) {
		$
			.post(
					'<portlet:resourceURL id="getCommonChartData"></portlet:resourceURL>',
					{
						...
						var chartColor_5 = "${tbinfo.chartColors5}";//颜色列表
						...
						var chartRadius_5 = "${tbinfo.radiusList}"; // 曲线点半径列表,默认为1
						...
						//该方法写在\tomcat-5.0.29\webapps\ROOT\html\company\scripts目录下的autoRenderToCharts.js中，用来生成图表
						autoRenderToMoreCharts("<portlet:namespace />consumption5",// 对应的渲染位置 
			...
			$.post(
					'<portlet:resourceURL id="getReportDataList"></portlet:resourceURL>',
					{
						...
						for(var i = 0 ;i<showCataList[0].length;i++){
							content +="<tr>" + (<portlet:namespace />choose_type5 == "day"?("<td>" + <portlet:namespace />start_date1 + "</td>"):"") + "<td>" + showCataList[0][i] + "</td>";
		...
		$(function() {
			...
			if(showBuild == "show"){
				$("#<portlet:namespace />choose_build5").show(); // 显示选择冷机项
			}else{ 
				$("#<portlet:namespace />choose_build5").hide(); // 隐藏选择冷机项
			}
			//调用图表加载方法，加载相应图表数据
		...
		function <portlet:namespace />loadData5(){
			...
			if($("#<portlet:namespace />table_chart_switch div:eq(0)").hasClass("btn_press")){
				if(<portlet:namespace />loadChart5 == 0){
					// 得到报表数据
					<portlet:namespace />getLeftChart5(<portlet:namespace />times5.join(),<portlet:namespace />chart_name5,
		...
		*/
		function <portlet:namespace />confirm5(){
			// 渲染左侧图表
			<portlet:namespace />getLeftChart5(<portlet:namespace />times5.join(),<portlet:namespace />chart_name5,<portlet:namespace />chart_id5,<portlet:namespace />chart_ispd5,<portlet:namespace />chartLegends5);
			// 获取网格数据
			<portlet:namespace />getForm5();
		}
