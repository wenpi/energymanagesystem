...
<div class="detail">
	<div>
		<table class = "build_energy_total">
...
<script type="text/javascript">
	...
	var <portlet:namespace/>choose_name1 = "${energyStatic.choose_name1}";//对应的数据name
	var <portlet:namespace/>choose_id1 = "${energyStatic.choose_id1}";//对应的数据Id
	var <portlet:namespace/>ispd1 = "${energyStatic.ispd1}";//对应的ispd
	var <portlet:namespace/>legendList1 = "${energyStatic.chartLegendList_1}";//图例列表
	...
	if(publicTime != ""){
		...
		if ("${energyStatic.defaulttime1}" == "") {//默认显示当前时间
			var nowdate = new Date();
			<portlet:namespace/>start_date1 = nowdate.getFullYear() + "-" + (nowdate.getMonth() + 1)
					+ "-" + nowdate.getDate();
		} else {
			<portlet:namespace/>start_date1 = "${energyStatic.defaulttime1}";//首选项数据，默认时间
		}
	}
	...
	WdatePicker({
		...
		onpicked : function(dp) {
			...
			if (type == "day") {
				...
				$(".build_energy_total td > span").text("日"); // 文字
				$(".build_energy_total > tbody > tr:eq(3) > td:eq(0)").text("同比昨日"); // 文字
			} else if (type == "week") {
				<portlet:namespace/>step1 = "1";
				<portlet:namespace/>outsideStr1 = "该周";
				$(".build_energy_total td > span").text("周"); // 文字
				$(".build_energy_total > tbody > tr:eq(3) > td:eq(0)").text("同比上周"); // 文字
			} else if (type == "month") {
				<portlet:namespace/>outsideStr1 = _time.getDateStr('yyyy') + "年"
						+ _time.getDateStr('MM') + "月";
				<portlet:namespace/>step1 = "5";
				$(".build_energy_total td > span").text("月"); // 文字
				$(".build_energy_total > tbody > tr:eq(3) > td:eq(0)").text("同比上月"); // 文字
			} else if (type == "year") {
				<portlet:namespace/>outsideStr1 = _time.getDateStr('yyyy') ;
				<portlet:namespace/>step1 = "2";
				$(".build_energy_total td > span").text("年"); // 文字
				$(".build_energy_total > tbody > tr:eq(3) > td:eq(0)").text("同比去年"); // 文字
			}
			<portlet:namespace/>choose_name1 = '${energyStatic.choose_name1}'; // 对应的name
			<portlet:namespace/>choose_id1 = '${energyStatic.choose_id1}'; // 对应的id
			<portlet:namespace/>ispd1 = '${energyStatic.ispd1}'; // 对应的ispd

			var times = "";
			// 根据name的个数，来为att添加对应个数的值
			var legend = "${energyStatic.chartLegendList_1}".replace(/'/g,"").split(",");//
			for ( var i = 0; i < legend.length; i++) {
				if(i<legend.length-1){
...
function <portlet:namespace/>comparing1(){
	...
	WdatePicker({
		...
		onpicked : function(dp) {
			...
			// 根据name的个数，来为att添加对应个数的值
			var chartLegendList = [${energyStatic.chartLegendList_1}];
			...
			<portlet:namespace />exp_start_date1 =times;// 导出到excel中的时间 
			<portlet:namespace />choose_name1 = '${energyStatic.choose_name1},${energyStatic.choose_name1}'; // 对应的name
			<portlet:namespace />choose_id1 = '${energyStatic.choose_id1},${energyStatic.choose_id1}'; // 对应的id
			<portlet:namespace />ispd1 = '${energyStatic.ispd1},${energyStatic.ispd1}'; // 对应的ispd
			for(var i=0;i<chartLegendList.length;i++){
...
function <portlet:namespace/>getChart1(tfrom, name, id, ispd, legend, showLegend,
	...
	$.post("<portlet:resourceURL id='getCommonChartData'></portlet:resourceURL>", {
		...
		id : id,
		ispd : ispd,
		decimals : '${energyStatic.decimals1}',//显示小数位数，首选项传
	...
	//选取时间段
		...
		if("${energyStatic.xStep_1}"!=''){
			<portlet:namespace/>step1="${energyStatic.xStep_1}";
		}
		//图例处理
		if(isNotCompare == "yes"){
			var legendList="${energyStatic.chartLegendList_1}".split(",");
			legend=legend.split(",");
			var temp=legend[1];
		...
		var chartColorList = [${energyStatic.chartColorList_1}];
		var chartYAxisList = [${energyStatic.chartYAxisList}];
		var chartTypeList=['${energyStatic.chartType_1}','${energyStatic.chartType_1_1}','${energyStatic.chartType_1}','${energyStatic.chartType_1_1}'];
		renderToBigTwoType("<portlet:namespace />chartArea1", 
				"${energyStatic.backgroundColor_1}",
				'',
				cateList,
				<portlet:namespace/>step1,
				"${energyStatic.yLeftTitle_1}",
				"${energyStatic.yRightTitle_1}",
				dataList,
				chartColorList,
				chartTypeList,
				chartYAxisList,
...
function <portlet:namespace/>expToExcel1(){
	...
	var expColumnName = "${energyStatic.expColumnName1}".replace(/'/g,"");
	// 导出excel的Url
	var expUrl = "<portlet:resourceURL id='expTotalEnergyDataToExcel'></portlet:resourceURL>"
			...
			+ isNotCompare
			+ "&expColumnName="
			+ (expColumnName==""?"${energyStatic.expColumnName1}".replace(/'/g,""):expColumnName);
	var removeUrl = '<portlet:resourceURL id="deleteExcelDataTempFile"><portlet:param name="fileName" value="${energyStatic.title1}" /></portlet:resourceURL>';
	var hSrc = "http://" + document.location.host
			+ "/energymanagesystem/uploadfiles/" + "${energyStatic.title1}"
			+ ".xls";
	$.ajax({
		type : "POST",
		url : expUrl,
...
function <portlet:namespace />getRightData1(){
	$.ajax({
		...
		success : function(result) {
			...
			$(".build_energy_total > tbody > tr:eq(0) > td:eq(1)").text(parseFloat(data[0][0])); // max
			$(".build_energy_total > tbody > tr:eq(0) > td:eq(3)").text(maxtime); // maxTime
			$(".build_energy_total > tbody > tr:eq(1) > td:eq(1)").text(parseFloat(data[1][0]));// min
			$(".build_energy_total > tbody > tr:eq(1) > td:eq(3)").text(mintime); // minTime
			$(".build_energy_total > tbody > tr:eq(2) > td:eq(1)").text(parseFloat(data[2][0])); // sum
			$(".build_energy_total > tbody > tr:eq(3) > td:eq(1)").text(returnUpOrDown(data[3][0]) + returnPositiveVal(data[3][0],"0"));// change
		},
		error : function(result) {
