...
<div class="detail">
	<div>
		<table class = "total_carbon">
			<caption>建筑碳足迹</caption>
			<tbody>
				<tr>
...
var <portlet:namespace/>choose_name6 = "${energyStatic.choose_name6}";//对应的数据name
var <portlet:namespace/>choose_id6 = "${energyStatic.choose_id6}";//对应的数据Id
var <portlet:namespace/>ispd6 = "${energyStatic.ispd6}";//对应的ispd
var <portlet:namespace/>legendList6 = "${energyStatic.chartLegendList_6}";//图例列表
var <portlet:namespace/>showLegend6 =${energyStatic.showLegend_6};//默认不显示图例
...
if(publicTime != ""){
	...
	if ("${energyStatic.defaulttime6}" == "") {//默认显示当前时间
		var nowdate = new Date();
		<portlet:namespace/>start_date6 = nowdate.getFullYear() + "-" + (nowdate.getMonth() + 1)
				+ "-" + nowdate.getDate();
	} else {
		<portlet:namespace/>start_date6 = "${energyStatic.defaulttime6}";//首选项数据，默认时间
	}
}
...
<portlet:namespace/>getChart6(times06, <portlet:namespace/>choose_name6, <portlet:namespace/>choose_id6, <portlet:namespace/>ispd6, <portlet:namespace/>legendList6, <portlet:namespace/>showLegend6, 'no');
//---------------------------------------------------------------------
//选择时间（日、周、月、年）
	...
	WdatePicker({
		...
		onpicked : function(dp) {
			...
			if (type == "day") {
				...
				$(".total_carbon td > span").text("日"); // 文字
				$(".total_carbon > tbody > tr:eq(3) > td:eq(0)").text("同比昨日"); // 文字
			} else if (type == "week") {
				<portlet:namespace/>step6 = "1";
				<portlet:namespace/>outsideStr6 = "该周";
				$(".total_carbon td > span").text("周"); // 文字
				$(".total_carbon > tbody > tr:eq(3) > td:eq(0)").text("同比上周"); // 文字
			} else if (type == "month") {
				<portlet:namespace/>outsideStr6 = _time.getDateStr('yyyy') + "年"
						+ _time.getDateStr('MM') + "月";
				<portlet:namespace/>step6 = "5";
				$(".total_carbon td > span").text("月"); // 文字
				$(".total_carbon > tbody > tr:eq(3) > td:eq(0)").text("同比上月"); // 文字
			} else if (type == "year") {
				<portlet:namespace/>outsideStr6 = _time.getDateStr('yyyy');
				$(".total_carbon td > span").text("年"); // 文字
				$(".total_carbon > tbody > tr:eq(3) > td:eq(0)").text("同比去年"); // 文字
				<portlet:namespace/>step6 = "2";
			}
			<portlet:namespace/>choose_name6 = '${energyStatic.choose_name6}'; // 对应的name
			<portlet:namespace/>choose_id6= '${energyStatic.choose_id6}'; // 对应的id
			<portlet:namespace/>ispd6 = '${energyStatic.ispd6}'; // 对应的ispd

			var times = "";
			// 根据name的个数，来为att添加对应个数的值
			var legend = <portlet:namespace/>legendList6.split(",");//
			...
			// 获取图表
			//数据类型String,String,String,String,Array,Boolean,String
			<portlet:namespace/>getChart6(times, <portlet:namespace/>choose_name6, <portlet:namespace/>choose_id6, <portlet:namespace/>ispd6,"${energyStatic.chartLegendList_6}",
					${energyStatic.showLegend_6}, 'no');
		}
	});
}
//时间对比
function <portlet:namespace/>comparing6(){
	...
	WdatePicker({
		...
		onpicked : function(dp) {
			...
			if("${energyStatic.referenceLegend_6}" != ""){
				<portlet:namespace />legendList6 += ","+"${energyStatic.referenceLegend_6}"; 
			}
		
			<portlet:namespace />showLegend6 = true;
			
			<portlet:namespace />exp_start_date6 = <portlet:namespace />start_date6 + "," + paretime; // 对应的开始时间
			<portlet:namespace />choose_name6 = '${energyStatic.choose_name6},${energyStatic.choose_name6}'; // 对应的name
			<portlet:namespace />choose_id6 = '${energyStatic.choose_id6},${energyStatic.choose_id6}'; // 对应的id
			<portlet:namespace />ispd6 = '${energyStatic.ispd6},${energyStatic.ispd6}'; // 对应的ispd
			//加入对比
...
function <portlet:namespace/>getChart6(tfrom, name, id, ispd, legend, showLegend6,
	...
	$.post("<portlet:resourceURL id='getCommonChartData'></portlet:resourceURL>", {
		...
		id : id,
		ispd : ispd,
		decimals : '${energyStatic.decimals6}',//显示小数位数，首选项传
	...
	//选取时间段
		...
		seriesList = returnSeriesList("${energyStatic.chartType_6_1}",
									  "${energyStatic.connectLineWidth_6}", // 连接线线宽，为空则默认为2，为0则不显示连接线
									  "${energyStatic.lineWidth_6}", // 线宽，此为具体曲线点的点线宽  ，为空则默认为2
									  legend,
									  "${energyStatic.chartColorList_6}", // 颜色列表
									  "${energyStatic.chartFillColorList_6}", // 填充颜色列表
									  "${energyStatic.chartRadiusList_6}", // 曲线点半径列表,默认为4
									  "${energyStatic.symbol6}", // 曲线点类型："circle", "square", "diamond", "triangle","triangle-down"，默认是"circle"
									  "${energyStatic.referencevalue6}", // 参考值  
									  "${energyStatic.chartType_6_2}", // 参考值的图表类型
									  "${energyStatic.referenceLegend_6}", // 参考值对应的图例名称
									  "${energyStatic.referenceColor_6}", // 参考值对应的颜色
									  dataList);
		// 将图表渲染到对应的位置上
		if("${energyStatic.xStep_6}"!=""){
			<portlet:namespace/>step6="${energyStatic.xStep_6}";
		}
		...
		renderToBigCommonChart("<portlet:namespace />chartArea6", // 对应的渲染位置
						   ...
						   cateList, // X轴数据列表
						   '', // 列表中的线的样式，默认为虚线
						   '${energyStatic.yLeftTitle_6}', //  Y轴标题
						   '${energyStatic.ySymbol6}', // Y轴的单位
						   showLegend6 == '' ? false : showLegend6, // 是否显示图例
						   "${energyStatic.referenceColor_6}", // 参考颜色
						   seriesList,
							"${energyStatic.backgroundColor_6}");
	}, 'json');
}
//导出数据到excel文件
function <portlet:namespace/>expToExcel6(){
	...
	var expColumnName = "${energyStatic.expColumnName6}";
	// 导出excel的Url
	var expUrl = "<portlet:resourceURL id='expCommonChartDataToExcel'></portlet:resourceURL>"
			...
			+ isNotCompare
			+ "&expColumnName="
			+ (expColumnName==""?"${energyStatic.title6}":expColumnName);
	var removeUrl = '<portlet:resourceURL id="deleteExcelDataTempFile"><portlet:param name="fileName" value="${energyStatic.title6}" /></portlet:resourceURL>';
	var hSrc = "http://" + document.location.host
			+ "/energymanagesystem/uploadfiles/" + "${energyStatic.title6}"
			+ ".xls";
	$.ajax({
		type : "POST",
		url : expUrl,
...
function <portlet:namespace />getRightData6(){
	$.ajax({
		...
		success : function(result) {
			...
			$(".total_carbon > tbody > tr:eq(0) > td:eq(1)").text(parseFloat(data[0][0])); // max
			$(".total_carbon > tbody > tr:eq(0) > td:eq(3)").text(maxtime); // maxTime
			$(".total_carbon > tbody > tr:eq(1) > td:eq(1)").text(parseFloat(data[1][0]));// min
			$(".total_carbon > tbody > tr:eq(1) > td:eq(3)").text(mintime); // minTime
			$(".total_carbon > tbody > tr:eq(2) > td:eq(1)").text(parseFloat(data[2][0])); // sum
			$(".total_carbon > tbody > tr:eq(3) > td:eq(1)").text(returnUpOrDown(data[3][0]) + returnPositiveVal(data[3][0],"0"));// change
		},
		error : function(result) {
