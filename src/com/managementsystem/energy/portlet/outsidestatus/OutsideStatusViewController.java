package com.managementsystem.energy.portlet.outsidestatus;
...
@Controller
@RequestMapping("VIEW")
public class OutsideStatusViewController extends BaseController {
	...
	public String setup(PortletPreferences prefs, Model model) {
		...
		if(buildinfo.getBuildDistrictid()!=null){
			...
			districtinfo = districtService.getDistrictinfo(buildinfo.getBuildDistrictid());
		...
		if(buildinfo.getBuildStrutype()!=null){
			...
			dictitem = dictionaryService.getDictitemByItemcode("build_strutype", buildinfo.getBuildStrutype().toString());
		...
		if(buildinfo.getBuildFunction()!=null){
			Dictitem dictitem = null;
			dictitem = dictionaryService.getDictitemByItemcode("build_function", buildinfo.getBuildStrutype().toString());
			buildFunction = dictitem.getItemName();
			model.addAttribute("buildFunction", buildFunction);
		}
		if(buildinfo.getBuildAirtype()!=null){
			Dictitem dictitem = new Dictitem();
			dictitem = dictionaryService.getDictitemByItemcode("build_airtype", buildinfo.getBuildAirtype().toString());
			buildAirtype = dictitem.getItemName();
			model.addAttribute("buildAirtype", buildAirtype);
		}
		
		return PORTLETFOLDER + "outsidestatus/outsideStatusView";
	}
}
