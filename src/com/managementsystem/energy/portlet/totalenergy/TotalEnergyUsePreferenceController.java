...
@Controller
@RequestMapping("Edit")
public class TotalEnergyUsePreferenceController extends BaseController {

	private final Log logger = LogFactory
			.getLog(TotalEnergyUsePreferenceController.class); // 记录
	@Autowired
	...
	public ModelAndView setup(RenderRequest request, RenderResponse response,
		...
		if (prefs.getMap().size() == 0) {
			...
			try {
				energyStaticPreferenceInfo = (EnergyStaticPreferenceInfo)prefernceInfoService.getPreferenceInfo(prefs,new EnergyStaticPreferenceInfo());
			} catch (Exception e) {
				e.printStackTrace();
			}
		}

		mav.addObject("totalEnergy", energyStaticPreferenceInfo);
		mav.setViewName(PORTLETFOLDER + "totalenergyuse/totalEnergyUsePreferenceChart");// 返回路径
		return mav;
	}
