...
@Controller
@RequestMapping("Edit")
public class RunningMonitorChartPreferenceController extends BaseController {
	...
	public ModelAndView setup(RenderRequest request, RenderResponse response,
		...
		if (prefs.getMap().size() != 0) {
			navigationPreferenceInfo = new NavigationPreferenceInfo();
		} else {
			try {
		...
		mav.setViewName(PORTLETFOLDER + "othermodules/runningMonitorInfoPreferences");//返回一个视图，并保存进入ModelAndView对象中
		return mav;
	}
